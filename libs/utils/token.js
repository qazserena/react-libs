...
var cache = new StorageItem('cache-api-token');
var tokenInMemory = cache.data;
export function setApiToken(token, cacheIt) {
    if (cacheIt === void 0) { cacheIt = false; }
    tokenInMemory = token;
    if (cacheIt) {
        cache.data = token;
    }
...
var tokenExpireHandler = null;
/**
 * 设置token过期的处理函数
 */
export function setTokenExpireHandler(handler) {
    tokenExpireHandler = handler;
}
request.interceptors.request.use(function (url, options) {
    if (tokenInMemory) {
        options.headers = __assign(__assign({}, options.headers), { 'x-api-token': tokenInMemory });
    }
...
request.interceptors.response.use(function (response) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!(response.status === 403)) return [3 /*break*/, 4];
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
